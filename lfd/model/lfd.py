...
class LFD(nn.Module):
    ...
    def predict_for_single_image(self, image, aug_pipeline, classification_threshold=None, nms_threshold=None, class_agnostic=False, device_index=0):
        ...
        data_batch = data_batch[None]  # Add batch dimension
        data_batch = data_batch.transpose([0, 3, 1, 2])  # Convert HWC to CHW format
        data_batch = torch.from_numpy(data_batch).to(device)  # Move directly to target device
        ...
        predicted_classification = predicted_classification[0]  # Remove batch dimension
        predicted_regression = predicted_regression[0]  # Remove batch dimension

        # Generate point coordinates on CPU (avoid creating tensors on GPU)
        all_point_coordinates_list = self.generate_point_coordinates(self._head_indexes_to_feature_map_sizes)
        expanded_regression_ranges_list = [
            all_point_coordinates_list[i].new_tensor(self._regression_ranges[i])[None].expand_as(all_point_coordinates_list[i])
            for i in range(self._num_heads)
        ]

        concat_point_coordinates = torch.cat(all_point_coordinates_list, dim=0).to(device)
        concat_regression_ranges = torch.cat(expanded_regression_ranges_list, dim=0).to(device)

        # Process classification scores based on loss type
        if type(self._classification_loss_func).__name__ in ['CrossEntropyLoss']:
            predicted_classification = predicted_classification.softmax(dim=1)
            predicted_classification = predicted_classification[:, :-1]  # Remove background class
        else:
            predicted_classification = predicted_classification.sigmoid()

        # Apply classification threshold filtering
        ...
        if selected_indexes.numel() == 0:
            return []  # No detections meet threshold

        # Filter predictions using selected indexes
        ...
        # Calculate bounding boxes based on regression type
        if self._regression_loss_type == 'independent':
        ...
        # Update NMS configuration if parameters are provided
        if nms_threshold:
        ...
        # Convert [x1, y1, x2, y2, score] to [x1, y1, w, h, score]
        nms_bboxes[:, 2] = nms_bboxes[:, 2] - nms_bboxes[:, 0] + 1  # Calculate width
        nms_bboxes[:, 3] = nms_bboxes[:, 3] - nms_bboxes[:, 1] + 1  # Calculate height
        
        # Format results: [class_label, score, x1, y1, w, h]
        results = torch.cat([nms_labels[:, None].to(nms_bboxes), nms_bboxes[:, [4, 0, 1, 2, 3]]], dim=1)
        
        # Convert to list format
        results = results.cpu().numpy() if device.type == 'cuda' else results.numpy()
        results = [[int(temp_result[0])] + temp_result[1:].tolist() for temp_result in results]
