...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("nms", &nms, "non-maximum suppression (CPU only)");
    m.def("soft_nms", &soft_nms, "soft non-maximum suppression (CPU only)");
    m.def("nms_match", &nms_match, "non-maximum suppression match (CPU only)");
}
