...
def prepare_common_settings():
    ...
    sys.excepthook = customize_exception_hook(
        os.path.join(config_dict['work_dir'], 'exception_log_' + config_dict['timestamp'] + '.log'))
...
def prepare_optimizer():
    ...
    assert max(config_dict['milestones']) < config_dict[
        'training_epochs'], 'the max value in milestones should be less than total epochs!'

    config_dict['lr_scheduler'] = torch.optim.lr_scheduler.MultiStepLR(config_dict['optimizer'],
                                                                       milestones=config_dict['milestones'],
                                                                       gamma=config_dict[
                                                                           'gamma'])  # scheduler 也需要被保存在checkpoint中
    ...
    assert isinstance(config_dict['warmup_setting'], dict) and 'by_epoch' in config_dict[
        'warmup_setting'] and 'warmup_mode' in config_dict['warmup_setting'] \
